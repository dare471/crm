...
//{ TEST API  }
    ...
    //api for history browsingList
    public function HistoryBrowsingList($user_id){
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->select("SELECT ID, USER_ID, REGION, MODE, DISTRICT, CLIENT_FIELDS FROM CRM_HISTORY_BROWSING WHERE USER_ID = $user_id");
        if(empty($query)){
            return response()->json([
                'succes' => false,
                'status' => 201,
    ...
    //api for history browsingDetail
    public function HistoryBrowsingDetail($id){
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->select("SELECT ID, USER_ID, REGION, MODE, DISTRICT, CLIENT_FIELDS FROM CRM_HISTORY_BROWSING WHERE ID = $id");
        if(empty($query)){
            return response()->json([
                'succes' => false,
                'status' => 201,
    ...
    //apu for create new element for tables history_browsing
    public function HistoryBrowsingCreate(Request $request){
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->table('CRM_HISTORY_BROWSING')
            ->insert([
...
//api for Add comment another content
    public function AddCommentContent(Request $request)
    {
        if ($request->FIELDS == '') {
            $dbconn = DB::connection('CRM_DWH');
            $query = $dbconn->table('CRM_COMMENT_ELEMENT')
                ->insert([
                    'ELEMENT_ID' => $request->ELEMENT_ID,
                    'DESCRIPTION' => $request->DESCRIPTION,
                    'CREATED_BY' => $request->USERS_ID,
                    'CATEGORY_CHAPTER_ID' => $request->CATEGORY_ID,
                    'TYPE' => $request->TYPE,
                    // type = 1 it's just comment, if type=2 it's answer for commnet
                    'ANSWER_ID' => $request->COMMENT_ID,
                    // 'FIELDS_ID' => $request->FIELDS_ID
                    //or if answer for comment to add property :: 'ANDSWER_ID' => $request->COMMENT_ID
                ]);
            return response()->json([
                'success' => true,
                'status' => 201,
...
//api for views list comment to content
    public function ListCommentContent(Request $request)
    {
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->table('CRM_COMMENT_ELEMENT')
            ->select('*')
            ->where('ELEMENT_ID', $request->ELEMENT_ID)
            ->where('CATEGORY_CHAPTER_ID', $request->CATEGORY_ID)
            ->get();
        return response($query);
    }
...
//api for delete comment to content
    public function DeleteCommentContent(Request $request)
    {
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->table('CRM_COMMENT_ELEMENT')
            ->select('*')
        ...
        return response()->json([
            'status' => true,
            'message' => "that's $request->COMMENT_ID element Deleted"
        ]);
    }


//api for update comment to content
    public function UpdateCommentContent(Request $request)
    {
        ...
        return response()->json([
            'status' => true,
            'message' => "that's $request->COMMENT_ID updated"
        ]);
    }


/// для работы с таблицой элеватора api 

    public function WorkTable()
    {
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->select("SELECT ID, NAME, BIN, LOCATION, STATION, CONTACTS, STORAGE_VOLUME
        FROM CRM_DWH.dbo.CRM_ELEVATOR");
        return response($query);
    }



    public function ElementElevator($id)
    {
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->select("SELECT ID, NAME, BIN, LOCATION, STATION, CONTACTS, STORAGE_VOLUME
        FROM CRM_DWH.dbo.CRM_ELEVATOR WHERE ID = $id");
        return response($query);
    }
    ...
    {
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->table("CRM_ELEVATOR")
        ->select("*")
        ->where("ID", $request->id)
        ->update([
            'CONTACTS' => $request->contacts_cl, 
        ]);
        return response($query);
    }
    public function UpdateDataTableSV(Request $request)
    {
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->table("CRM_ELEVATOR")
        ->select("*")
    ...
    public function ElementElevatorAdd(Request $request)
    {
        $dbconn = DB::connection('CRM_DWH');
        $query = $dbconn->table("CRM_ELEVATOR")
            ->insert(['NAME' => $request->name_add, 
            'BIN' => $request->bin_add, 
            'LOCATION' => $request->location_add, 
            'STATION' => $request->station_add, 
            'CONTACTS' => $request->contacts_add, 
            'STORAGE_VOLUME' => $request->volume_add]);
        return response($query);
    }
